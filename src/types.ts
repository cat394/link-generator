...
/**
 * A type that formats objects of intersection types.
 * 
 * This was created based on this post by Colin McDonnell.
 * 
 * @see https://x.com/colinhacks/status/1835675253432881162
 * 
 * @example
 * type A = { key1: string };
 * type B = { key2: number };
 * 
 * type Result = PrettifyIntersectionObje<A & B>;
 * // Result = { key1: string, key2: number };
 * */ 
type PrettifyIntersectionObj<T> = Omit<T, never>;
...
type ParamDoesNotExist<T> = IsType<unknown, T>;
...
type ExtractRouteData<FlattenedRoutes extends FlatRouteConfig> = {
	[RouteId in keyof FlattenedRoutes]: {
		...
		params: ParamDoesNotExist<PathParams<FlattenedRoutes[RouteId]>> extends true
			? never
			: PrettifyIntersectionObj<PathParams<FlattenedRoutes[RouteId]>>;
		query: ParamDoesNotExist<QueryParams<FlattenedRoutes[RouteId]>> extends true
			? never
			: Partial<PrettifyIntersectionObj<QueryParams<FlattenedRoutes[RouteId]>>>;
	};
};
...
> = ExtractRouteData<Config>[RouteId]["params"] extends EmptyObject
	...
	: ParamDoesNotExist<ExtractRouteData<Config>[RouteId]["params"]> extends true
	? [undefined?, ExtractRouteData<Config>[RouteId]["query"]?]
	: [
