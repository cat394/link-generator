...
type ConvertStringType<UnionSegment extends string> =
	UnionSegment extends "string" ? string : never;

type ConvertNumberType<UnionSegment extends string> =
	UnionSegment extends "number"
		? number
		: UnionSegment extends `${infer NumericString extends number}`
			? NumericString
			: UnionSegment;

type ConvertBooleanType<UnionSegment extends string> =
	UnionSegment extends "boolean"
		? boolean
		: UnionSegment extends "true"
			? true
...
type InferUnionType<UnionPart extends string> =
	UnionPart extends `${Symbols.ConvertString}${infer StringToConvert}`
...
type ExtractParamArea<Segment> =
	Segment extends `${Symbols.PathParam}${infer ParamArea}`
		? ParamArea extends `${infer ParamAreaExcludedQueryArea}${Symbols.Query}${string}`
...
type QueryParams<Path extends string> =
	PathNotContainParams<
...
> = Config extends RouteConfig
	? {
			[RouteId in CreateRouteId<Config>]: RouteId extends `${infer ParentRouteId}${Symbols.PathSeparater}${infer ChildrenRouteId}`
				? FlattenRouteConfig<
						Config[ParentRouteId]["children"],
 ...
 * // }
 */
type FlatRouteConfigRemovedParentQueryArea<Config> =
	Config extends FlatRouteConfig
		? {
				[RouteId in keyof Config]: Config[RouteId] extends `${infer Protocol}:/${infer Rest}`
...
type RemoveQueryArea<Path extends string> =
	Path extends `${infer RoutePath}${Symbols.Query}${string}` ? RoutePath : Path;
