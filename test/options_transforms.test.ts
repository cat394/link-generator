...
Deno.test("params context ", () => {
  const params_context = new ParamsContext({ id: "alice", section: "profile" });

  assertEquals(params_context.get("id"), "alice");
  assertEquals(params_context.get("section"), "profile");
  assertEquals(params_context.get("nonexistent"), undefined);
});
